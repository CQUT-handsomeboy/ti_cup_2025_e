...
//	DL_TimerG_startCounter(TIMER_0_INST);
	unsigned int LineL1 = 1, LineL2 = 1, LineR1 = 1, LineR2 = 1;//四路巡线模块初始值
	board_init();//相关初始化
	sprintf(buf,"Four_way patrol line\n");//四路巡线模块
	uart0_send_string(buf);//串口发送函数
...
//		line_patrol();
//		delay_ms(100);
			LineR1 = DL_GPIO_readPins(GRAY_PORT, GRAY_X4_PIN) > 0 ? 1 : 0;//读取右一
			LineR2 = DL_GPIO_readPins(GRAY_PORT, GRAY_X3_PIN) > 0 ? 1 : 0;//读取右二
			LineL2 = DL_GPIO_readPins(GRAY_PORT, GRAY_X1_PIN) > 0 ? 1 : 0;//读取左二
			LineL1 = DL_GPIO_readPins(GRAY_PORT, GRAY_X2_PIN) > 0 ? 1 : 0;//读取左一		
